...
⡝⡵⡕⡀⠑⠳⠿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠿⠛⢉⡠⡲⡫⡪⡪⡣ `);
            ...
            await sleep(4000);
            const code = await kyami.requestPairingCode(global.pairing);
            console.log('Process...');
...
kyami.ev.on('messages.upsert', async chatUpdate => {
    try {
        ...
        if (!kyami.public && !kay.key.fromMe && chatUpdate.type === 'notify') return;
        if (kay.key.id.startsWith('BAE5') && kay.key.id.length === 16) return;
        const messageId = kay.key.id;
        if (processedMessages.has(messageId)) return;
        processedMessages.add(messageId);
        const m = smsg(kyami, kay, store);
...
kyami.ev.on("connection.update",async  (s) => {
const { connection, lastDisconnect } = s
if (connection == "open") {
lolcatjs.fromString(`▧  Information connect :
│ » User id: ${kyami.user.id}
│ » Name: ${kyami.user.name}
└───···`)
        }
        if (
            connection === "close" &&
...
kyami.sendTextWithMentions = async (jid, text, quoted, options = {}) => {
...
kyami.sendImage = async (e, t, a = "", s = "", f) => {
    let r = Buffer.isBuffer(t)
      ...
      ? fs.readFileSync(t)
      : Buffer.alloc(0);
    return await kyami.sendMessage(
      e,
      { image: r, caption: a, ...f },
      { quoted: s }
    );
...
kyami.sendTextWithMentions = async (jid, text, quoted, options = {}) => kyami.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
return kyami;
}
